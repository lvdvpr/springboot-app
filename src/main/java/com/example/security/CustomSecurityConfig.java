...
public class CustomSecurityConfig extends WebSecurityConfigurerAdapter {
	 ...
	 * 1. HTTP 요청에 대한 인가 정책(권한체크 정책)을 설정한다. 
	 ...
	 * HttpSecurity
	 *   - 각종 보안관련 설정을 담당하는 객체
	 *   - HttpSecurity는 각종 xxxConfigurer 객체를 반환하는 메소드를 제공한다.
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
			// CSRF 토큰 사용여부 설정
			//.csrf()      // CSRF(사이트 간 요청위조) 토큰 사용여부 설정, CsrfConfigurer 객체를 반환한다.
			...
			.antMatchers("/", "/register", "/registered", "/login").permitAll()  // 접근권한 보유 여부를 확인하지 않고 접근을 허용한다. 인증되지 않아도 전부다 접근 허용
	...
	// 사용자정의 CustomUserDetailsService객체와 이 애플리케이션에서 사용하는 비밀번호 인코더를 AuthenticationManager인증관리자에 등록시킨다.
