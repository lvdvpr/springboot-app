...
public class CustomUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String userId) throws UsernameNotFoundException {
		...
		// 조회된 권한정보로 스프링에서 제공해주는 GrantedAuthority객체를 생성한다.
	...
	private Collection<? extends GrantedAuthority> getAuthorities(List<UserRole> userRoles) {
		...
		for (UserRole userRole : userRoles) {
			SimpleGrantedAuthority authority = new SimpleGrantedAuthority(userRole.getRoleName());  /// SimpleGrantedAuthority는 GrantedAuthority를 구현한 객체이다.
